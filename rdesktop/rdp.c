...
void rdp_out_unistr(STREAM s, char *string, int len) {
	...
	if (g_iconv_works) {
		if (iconv_h == (iconv_t) - 1) {
			size_t i = 1, o = 4;
			if ((iconv_h = iconv_open(WINDOWS_CODEPAGE, g_codepage))
					== (iconv_t) - 1) {
...
int rdp_in_unistr(STREAM s, char *string, int str_size, int in_len) {
	...
	if (g_iconv_works) {
		if (iconv_h == (iconv_t) - 1) {
			if ((iconv_h = iconv_open(g_codepage, WINDOWS_CODEPAGE))
					== (iconv_t) - 1) {
...
static void rdp_send_logon_info(uint32 flags, char *domain, char *user,
	...
	if (!g_use_rdp5 || 1 == g_server_rdp_version) {
		...
		/* clientSessionId, consider changing to 0 */
