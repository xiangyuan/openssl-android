...
RD_HBITMAP cache_get_bitmap(uint8 id, uint16 idx) {
	if ((id < NUM_ELEMENTS(g_bmpcache))
			&& (idx < NUM_ELEMENTS(g_bmpcache[0]))) {
	...
	if ((id < NUM_ELEMENTS(g_bmpcache))
		...
		if (IS_PERSISTENT(id)) {
			if (old == NULL
				...
				g_bmpcache[id][idx].previous = g_bmpcache[id][idx].next =
						NOT_SET;
...
cache_get_font(uint8 font, uint16 character) {
	...
	if ((font < NUM_ELEMENTS(g_fontcache))
			&& (character < NUM_ELEMENTS(g_fontcache[0]))) {
