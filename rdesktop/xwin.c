...
static void seamless_XFillPolygon(Drawable d, XPoint * points, int npoints,
		int xoffset, int yoffset) {
	points[0].x -= xoffset;
	points[0].y -= yoffset;
	XFillPolygon(g_display, d, g_gc, points, npoints, Complex,
			CoordModePrevious);
	points[0].x += xoffset;
	points[0].y += yoffset;
}

static void seamless_XDrawLines(Drawable d, XPoint * points, int npoints,
		int xoffset, int yoffset) {
	points[0].x -= xoffset;
	points[0].y -= yoffset;
...
static void sw_remove_window(seamless_window * win) {
...
static void sw_all_to_desktop(Window wnd, unsigned int desktop) {
	seamless_window *sw;
	for (sw = g_seamless_windows; sw; sw = sw->next) {
		if (sw->wnd == wnd)
			continue;
...
/* Send our position */
static void sw_update_position(seamless_window * sw) {
	...
	XTranslateCoordinates(g_display, sw->wnd, wa.root, -wa.border_width,
			-wa.border_width, &x, &y, &child_return);
...
static void sw_check_timers() {
	...
	for (sw = g_seamless_windows; sw; sw = sw->next) {
		if (timerisset(sw->position_timer)
				&& timercmp(sw->position_timer, &now, <))
				{
...
static void sw_restack_window(seamless_window * sw, unsigned long behind) {
...
static void sw_handle_restack(seamless_window * sw) {
	Status status;
	Window root, parent, *children;
	unsigned int nchildren, i;
	seamless_window *sw_below;

	status = XQueryTree(g_display, RootWindowOfScreen(g_screen), &root, &parent,
			&children, &nchildren);
	if (!status || !nchildren)
		return;
...
sw_find_group(unsigned long id, RD_BOOL dont_create) {
	...
	sg->wnd = XCreateWindow(g_display, RootWindowOfScreen(g_screen), -1, -1, 1,
			1, 0, CopyFromParent, CopyFromParent, CopyFromParent, 0, &attribs);
...
static void mwm_hide_decorations(Window wnd) {
...
static Bool sw_configurenotify_p(Display * display, XEvent * xevent,
		XPointer arg) {
...
static void sw_wait_configurenotify(Window wnd, unsigned long serial) {
	XEvent xevent;
	...
	do {
		if (XCheckIfEvent(g_display, &xevent, sw_configurenotify_p,
				(XPointer) &context)) {
			got = True;
			break;
		}
...
static Window sw_get_toplevel(Window wnd) {
	Window root, parent;
	Window *child_list;
	...
	while (1) {
		...
		if (root == parent) {
			...
			warning(
					"Internal error: sw_get_toplevel called with root window\n");
		}
...
static RD_BOOL sw_window_is_behind(Window wnd, Window behind) {
	...
	XQueryTree(g_display, RootWindowOfScreen(g_screen), &dummy1, &dummy2,
			&child_list, &num_children);
...
static void seamless_restack_test() {
	...
	for (i = 0; i < 3; i++) {
		char name[64];
		wnds[i] = XCreateSimpleWindow(g_display, RootWindowOfScreen(g_screen),
				0, 0, 20, 20, 0, 0, 0);
	...
	if (!sw_window_is_behind(wnds[0], wnds[1])
			|| !sw_window_is_behind(wnds[2], wnds[1])) {
	...
	XReconfigureWMWindow(g_display, wnds[1], DefaultScreen(g_display),
...
static uint32 translate_colour(uint32 colour) {
	PixelColour pc;
...
static void translate8to8(const uint8 * data, uint8 * out, uint8 * end) {
	while (out < end)
		*(out++) = (uint8) g_colmap[*(data++)];
}

static void translate8to16(const uint8 * data, uint8 * out, uint8 * end) {
	uint16 value;

	if (g_compatible_arch) {
		/* *INDENT-OFF* */
		REPEAT2( *((uint16 *) out) = g_colmap[*(data++)]; out += 2;)
		/* *INDENT-ON* */
...
static void translate8to24(const uint8 * data, uint8 * out, uint8 * end) {
	uint32 value;
...
static void translate8to32(const uint8 * data, uint8 * out, uint8 * end) {
	uint32 value;

	if (g_compatible_arch) {
		/* *INDENT-OFF* */
		REPEAT4( *((uint32 *) out) = g_colmap[*(data++)]; out += 4;)
		/* *INDENT-ON* */
	} else if (g_xserver_be) {
		while (out < end) {
			value = g_colmap[*(data++)];
...
static void translate15to16(const uint16 * data, uint8 * out, uint8 * end) {
	uint16 pixel;
	uint16 value;
	PixelColour pc;
...
static void translate15to24(const uint16 * data, uint8 * out, uint8 * end) {
	uint32 value;
	uint16 pixel;
	PixelColour pc;
	...
	if (g_compatible_arch) {
		/* *INDENT-OFF* */
		REPEAT3(
				pixel = *(data++); SPLITCOLOUR15(pixel, pc); *(out++) = pc.blue; *(out++) = pc.green; *(out++) = pc.red;)
		/* *INDENT-ON* */
	} else if (g_xserver_be) {
		while (out < end) {
			pixel = *(data++);
...
static void translate15to32(const uint16 * data, uint8 * out, uint8 * end) {
	uint16 pixel;
	uint32 value;
	PixelColour pc;
	...
	if (g_compatible_arch) {
		/* *INDENT-OFF* */
		REPEAT4(
				pixel = *(data++); SPLITCOLOUR15(pixel, pc); *(out++) = pc.blue; *(out++) = pc.green; *(out++) = pc.red; *(out++) = 0;)
		/* *INDENT-ON* */
	} else if (g_xserver_be) {
		while (out < end) {
			pixel = *(data++);
...
static void translate16to16(const uint16 * data, uint8 * out, uint8 * end) {
	uint16 pixel;
	uint16 value;
	PixelColour pc;
...
static void translate16to24(const uint16 * data, uint8 * out, uint8 * end) {
	uint32 value;
	uint16 pixel;
	PixelColour pc;
	...
	if (g_compatible_arch) {
		/* *INDENT-OFF* */
		REPEAT3(
				pixel = *(data++); SPLITCOLOUR16(pixel, pc); *(out++) = pc.blue; *(out++) = pc.green; *(out++) = pc.red;)
		/* *INDENT-ON* */
	} else if (g_xserver_be) {
		if (g_host_be) {
			while (out < end) {
...
static void translate16to32(const uint16 * data, uint8 * out, uint8 * end) {
	uint16 pixel;
	uint32 value;
	PixelColour pc;
	...
	if (g_compatible_arch) {
		/* *INDENT-OFF* */
		REPEAT4(
				pixel = *(data++); SPLITCOLOUR16(pixel, pc); *(out++) = pc.blue; *(out++) = pc.green; *(out++) = pc.red; *(out++) = 0;)
		/* *INDENT-ON* */
	} else if (g_xserver_be) {
		if (g_host_be) {
			while (out < end) {
...
static void translate24to16(const uint8 * data, uint8 * out, uint8 * end) {
	uint32 pixel = 0;
	uint16 value;
	PixelColour pc;
...
static void translate24to24(const uint8 * data, uint8 * out, uint8 * end) {
	uint32 pixel;
	uint32 value;
	PixelColour pc;
...
static void translate24to32(const uint8 * data, uint8 * out, uint8 * end) {
	uint32 pixel;
	uint32 value;
	PixelColour pc;
	...
	if (g_compatible_arch) {
		...
		REPEAT4(
				...
				*((uint32 *) out) = *((uint16 *) data) + (*((uint8 *) data + 2) << 16); out += 4; data += 3;)
#endif
		/* *INDENT-ON* */
	} else if (g_xserver_be) {
		while (out < end) {
...
static void xwin_refresh_pointer_map(void) {
...
static RD_BOOL select_visual(int screen_num) {
	...
	vmatches = XGetVisualInfo(g_display, VisualClassMask | VisualScreenMask
			, &template,
			&visuals_count);
	g_visual = NULL;
	...
	if (vmatches != NULL)
	{
		for (i = 0; i < visuals_count; ++i) {
			...
			if (can_translate_to_bpp) {
				unsigned vis_red_weight = calculate_mask_weight(
						visual_info->red_mask);
				unsigned vis_green_weight = calculate_mask_weight(
						visual_info->green_mask);
				unsigned vis_blue_weight = calculate_mask_weight(
	...
	if (g_visual != NULL)
	{
		...
		calculate_shifts(g_visual->green_mask, &g_green_shift_r,
				&g_green_shift_l);
		...
		vmatches = XGetVisualInfo(g_display,
...
/* Check if the X11 window corresponding to a seamless window with
 specified id exists. */
RD_BOOL sw_window_exists(unsigned long id) {
	seamless_window *sw;
	char *name;
	Status sts = 0;
...
static int error_handler(Display * dpy, XErrorEvent * eev) {
	if (g_error_expected)
		return 0;
...
RD_BOOL ui_init(void) {
	...
	if (!g_owncolmap) {
		g_xcolmap = XCreateColormap(g_display, RootWindowOfScreen(g_screen),
				g_visual, AllocNone);
		if (g_depth <= 8)
			warning(
...
static void get_window_attribs(XSetWindowAttributes * attribs) {
...
static void get_input_mask(long *input_mask) {
	*input_mask = KeyPressMask | KeyReleaseMask | ButtonPressMask
			| ButtonReleaseMask | VisibilityChangeMask | FocusChangeMask
			| StructureNotifyMask;
...
RD_BOOL ui_create_window(void) {
	...
	if ((g_ownbackstore) && (g_backstore == 0)) {
		g_backstore = XCreatePixmap(g_display, g_wnd, g_width, g_height,
				g_depth);
	...
	if (g_IM != NULL)
	{
		g_IC = XCreateIC(g_IM, XNInputStyle,
				(XIMPreeditNothing | XIMStatusNothing),
				XNClientWindow, g_wnd, XNFocusWindow, g_wnd, NULL);

		if ((g_IC != NULL)
				&& (XGetICValues(g_IC, XNFilterEvents, &ic_input_mask, NULL)
						== NULL))
	...
	if (g_null_cursor == NULL
		...
		g_null_cursor = ui_create_cursor(0, 0, 1, 1, null_pointer_mask,
				null_pointer_data, 24);
...
void ui_resize_window() {
	...
	if (g_backstore != 0) {
		...
		XCopyArea(g_display, g_backstore, bs, g_gc, 0, 0, g_width, g_height, 0,
				0);
...
void xwin_toggle_fullscreen(void) {
	...
	if (!g_ownbackstore) {
		...
		XCopyArea(g_display, g_wnd, contents, g_gc, 0, 0, g_width, g_height, 0,
				0);
	}
	...
	if (!g_ownbackstore) {
		XCopyArea(g_display, contents, g_wnd, g_gc, 0, 0, g_width, g_height, 0,
				0);
		XFreePixmap(g_display, contents);
	}
}

static void handle_button_event(XEvent xevent, RD_BOOL down) {
	...
	if (xevent.xmotion.window == g_wnd) {
		rdp_send_input(time(NULL), RDP_INPUT_MOUSE, flags | button,
...
static int xwin_process_events(void) {
	XEvent xevent;
	KeySym keysym;
	uint32 ev_time;
	char str[256];
	...
	while ((XPending(g_display) > 0) && events++ < 20) {
		...
		case KeyPress:
			...
			/* Multi_key compatible version */
			{
				...
				XLookupString((XKeyEvent *) &xevent, str, sizeof(str), &keysym,
						NULL);
			}
		...
		case FocusIn:
			...
			if (g_grab_keyboard && g_mouse_in_wnd)
				XGrabKeyboard(g_display, g_wnd, True, GrabModeAsync,
		...
		case Expose:
			if (xevent.xexpose.window == g_wnd) {
				XCopyArea(g_display, g_backstore, xevent.xexpose.window, g_gc,
				...
				XCopyArea(g_display, g_backstore, xevent.xexpose.window, g_gc,
						...
						xevent.xexpose.y + sw->yoffset, xevent.xexpose.width,
...
RD_HBITMAP ui_create_bitmap(int width, int height, uint8 * data) {
	...
	XPutImage(g_display, bitmap, g_create_bitmap_gc, image, 0, 0, 0, 0, width,
			height);
...
void ui_paint_bitmap(int x, int y, int cx, int cy, int width, int height,
	...
	if (g_ownbackstore) {
		...
		ON_ALL_SEAMLESS_WINDOWS(
				XCopyArea,
				(g_display, g_backstore, sw->wnd, g_gc, x, y, cx, cy, x - sw->xoffset, y - sw->yoffset));
	} else {
		XPutImage(g_display, g_wnd, g_gc, image, 0, 0, x, y, cx, cy);
		ON_ALL_SEAMLESS_WINDOWS(
				XCopyArea,
				(g_display, g_wnd, sw->wnd, g_gc, x, y, cx, cy, x - sw->xoffset, y - sw->yoffset));
	}
...
static int get_next_xor_pixel(uint8 * xormask, int bpp, int *k) {
	int rv = 0;
	PixelColour pc;
	uint8 *s8;
	uint16 *s16;
...
RD_HCURSOR ui_create_cursor(unsigned int x, unsigned int y, int width,
	...
	xcursor = XCreatePixmapCursor(g_display, (Pixmap) cursorglyph,
...
void ui_set_cursor(RD_HCURSOR cursor) {
	...
	XDefineCursor(g_display, g_wnd, g_current_cursor);
	ON_ALL_SEAMLESS_WINDOWS(XDefineCursor,
			(g_display, sw->wnd, g_current_cursor));
}
		...
		for (i = 0; i < ncolours; i++) {
			...
			if (XAllocColor(g_display, g_xcolmap, &xentry) == 0) {
				...
				while (colLookup--) {
					...
					XQueryColor(g_display, DefaultColormap(g_display,
							DefaultScreen(g_display)), &xc_cache[colLookup]);
				}
				colLookup = 0;
...
void ui_set_colourmap(RD_HCOLOURMAP map) {
	if (!g_owncolmap) {
		...
		ON_ALL_SEAMLESS_WINDOWS(XSetWindowColormap,
				(g_display, sw->wnd, (Colormap) map));
	}
}
...
static uint8 hatch_patterns[] = { 0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00,
		0x00, /* 0 - bsHorizontal */
...
/* brush */BRUSH * brush, int bgcolour, int fgcolour) {
	...
	ON_ALL_SEAMLESS_WINDOWS(
			XCopyArea,
			(g_display, g_ownbackstore ? g_backstore : g_wnd, sw->wnd, g_gc, x, y, cx, cy, x - sw->xoffset, y - sw->yoffset));
}
...
/* src */int srcx, int srcy) {
	SET_FUNCTION(opcode);
	if (g_ownbackstore) {
		XCopyArea(g_display, g_Unobscured ? g_wnd : g_backstore, g_wnd, g_gc,
				srcx, srcy, cx, cy, x, y);
		XCopyArea(g_display, g_backstore, g_backstore, g_gc, srcx, srcy, cx, cy,
				x, y);
	} else {
...
/* src */RD_HBITMAP src, int srcx, int srcy) {
	...
	ON_ALL_SEAMLESS_WINDOWS(
			XCopyArea,
			(g_display, (Pixmap) src, sw->wnd, g_gc, srcx, srcy, cx, cy, x - sw->xoffset, y - sw->yoffset));
	if (g_ownbackstore)
		XCopyArea(g_display, (Pixmap) src, g_backstore, g_gc, srcx, srcy, cx,
				cy, x, y);
	RESET_FUNCTION(opcode);
}
...
/* pen */PEN * pen) {
	...
	ON_ALL_SEAMLESS_WINDOWS(
			XDrawLine,
			(g_display, sw->wnd, g_gc, startx - sw->xoffset, starty - sw->yoffset, endx - sw->xoffset, endy - sw->yoffset));
	if (g_ownbackstore)
...
/* pen */PEN * pen) {
	...
	SET_FUNCTION(opcode);
	SET_FOREGROUND(pen->colour);
	XDrawLines(g_display, g_wnd, g_gc, (XPoint *) points, npoints,
			CoordModePrevious);
	if (g_ownbackstore)
...
/* dest */int x, int y, int cx, int cy,
/* src */RD_HGLYPH glyph, int srcx, int srcy, int bgcolour, int fgcolour) {
	SET_FOREGROUND(fgcolour);
	SET_BACKGROUND(bgcolour);
...
void ui_draw_text(uint8 font, uint8 flags, uint8 opcode, int mixmode, int x,
		int y, int clipx, int clipy, int clipcx, int clipcy, int boxx, int boxy,
	...
	if (g_ownbackstore) {
		if (boxcx > 1) {
			XCopyArea(g_display, g_backstore, g_wnd, g_gc, boxx, boxy, boxcx,
					boxcy, boxx, boxy);
			ON_ALL_SEAMLESS_WINDOWS(
					XCopyArea,
					(g_display, g_backstore, sw->wnd, g_gc, boxx, boxy, boxcx, boxcy, boxx - sw->xoffset, boxy - sw->yoffset));
		} else {
			XCopyArea(g_display, g_backstore, g_wnd, g_gc, clipx, clipy, clipcx,
					clipcy, clipx, clipy);
			ON_ALL_SEAMLESS_WINDOWS(
					XCopyArea,
					(g_display, g_backstore, sw->wnd, g_gc, clipx, clipy, clipcx, clipcy, clipx - sw->xoffset, clipy - sw->yoffset));
		}
	}
}
...
void ui_desktop_save(uint32 offset, int x, int y, int cx, int cy) {
	...
	if (g_ownbackstore) {
		image = XGetImage(g_display, g_backstore, x, y, cx, cy, AllPlanes,
				ZPixmap);
		exit_if_null(image);
	} else {
	...
	offset *= g_bpp / 8;
	cache_put_desktop(offset, cx, cy, image->bytes_per_line, g_bpp / 8,
			(uint8 *) image->data);
...
void ui_seamless_create_window(unsigned long id, unsigned long group,
	...
	get_window_attribs(&attribs);
	wnd = XCreateWindow(g_display, RootWindowOfScreen(g_screen), -1, -1, 1, 1,
			0, g_depth, InputOutput, g_visual,
			CWBackPixel | CWBackingStore | CWColormap | CWBorderPixel
			, &attribs);
...
void ui_seamless_seticon(unsigned long id, const char *format, int width,
		int height, int chunk, const char *data, int chunk_len) {
	seamless_window *sw;
...
void ui_seamless_delicon(unsigned long id, const char *format, int width,
		int height) {
	seamless_window *sw;
...
void ui_seamless_move_window(unsigned long id, int x, int y, int width,
	...
	XMoveResizeWindow(g_display, sw->wnd, sw->xoffset, sw->yoffset, sw->width,
			sw->height);
}
...
void ui_seamless_ack(unsigned int serial) {
	...
	for (sw = g_seamless_windows; sw; sw = sw->next) {
		if (sw->outstanding_position && (sw->outpos_serial == serial)) {
			...
			XCopyArea(g_display, g_backstore, sw->wnd, g_gc, sw->xoffset,
					sw->yoffset, sw->width, sw->height, 0, 0);
