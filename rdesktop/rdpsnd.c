...
static void rdpsnd_send(STREAM s) {
...
static void rdpsnd_flush_record(void) {
	STREAM s;
...
static void rdpsnd_clear_record(void) {
	/*
...
static RD_BOOL rdpsnd_auto_select(void) {
...
static void rdpsnd_process_negotiate(STREAM in) {
	...
	if (current_driver && !device_available
			&& current_driver->wave_out_open()) {
		current_driver->wave_out_close();
	...
	if (s_check_rem(in, 18 * in_format_count)) {
		for (i = 0; i < in_format_count; i++) {
			...
			if (current_driver
					&& current_driver->wave_out_format_supported(format)) {
				format_count++;
...
static void rdpsnd_process_packet(uint8 opcode, STREAM s) {
	...
	case RDPSND_WRITE:
		...
		rdpsnd_queue_write(
				rdpsnd_dsp_process(s->p, s->end - s->p, current_driver,
...
static void rdpsnd_process(STREAM s) {
	uint16 len;
	...
	while (!s_check_end(s)) {
		...
		if (packet.size == 0) {
			if ((s->end - s->p) < 4) {
				error(
						"RDPSND: Split at packet header. Things will go south from here...\n");
				return;
...
static RD_BOOL rdpsnddbg_line_handler(const char *line, void *data) {
...
static void rdpsnddbg_process(STREAM s) {
	unsigned int pkglen;
...
static void rdpsnd_register_drivers(char *options) {
...
static void rdpsnd_queue_complete_pending(void) {
	...
	while (queue_pending != queue_lo) {
		...
		elapsed = (packet->completion_tv.tv_sec - packet->arrive_tv.tv_sec)
				* 1000000
